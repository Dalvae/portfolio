...
6. Save TWO files to `job-responses/`:
   - `[company_name]_[role_name]_[YYYY-MM-DD].md` with markdown formatting
   - `[company_name]_[role_name]_[YYYY-MM-DD].txt` with plain text (no markdown) for easy clipboard paste
7. Copy the `.txt` file to clipboard: detect the session type first, then use the right tool:
   - If `$XDG_SESSION_TYPE` is `wayland` or `$HYPRLAND_INSTANCE_SIGNATURE` is set → `cat [file].txt | wl-copy`
   - Otherwise (X11 / i3) → `cat [file].txt | xclip -selection clipboard`
8. Render the `.md` file with `glow [file].md`
9. Inform the user the response is already in their clipboard.
